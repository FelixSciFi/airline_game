...
func _draw() -> void:
	...
	# 规划线：DESTINATION MODE 下 origin → selected
	...
	if selecting_destination and origin_city_id != "" and selected_city_id != "":
		...
		for line_off in line_offsets_x:
			...
			var p1: Vector2 = _map_view.world_to_screen(o_off, view_size).round()
			var p2: Vector2 = _map_view.world_to_screen(d_off, view_size).round()
	...
	for flight in flights:
		...
		for line_off in flight_line_offsets:
			...
			var fp1: Vector2 = _map_view.world_to_screen(o_off, view_size).round()
			var fp2: Vector2 = _map_view.world_to_screen(d_off, view_size).round()
